...
def capturar_rostro():
    try:
        ...
        modelo_path = f"modelos/modelo_user_{user_id}.xml"
...
def chat():
    ...
    if request.method == "POST":
        ...
        # FIX: typo corregido (tenías "pregqueda")
        ...
        cur = conn.cursor()
        cur.execute("""
            INSERT INTO Historial (user_id, pregunta, respuesta, fecha)
            VALUES (%s, %s, %s, NOW())
        """, (session["user"]["id"], pregunta, respuesta))
        conn.commit()
        cur.close()
        conn.close()
